...
def enc(parameters, context):  # 对模型参数加密的函数   #为什么这里是context
    for var in parameters:
...
if __name__ == "__main__":
    ...
    if args['dataset'] == 'cifar10':
        ...
        print("优化算法：", opti)
    # 创建Clients群
    myClients = ClientsGroup(args['dataset'], args['IID'], args['num_of_clients'], dev)#这里的iid是什么意思
...
#此时已经加密了
    ...
    for i in range(args['num_comm']):
        ...
        if args['algorithm'] == 'FLAvg':
            order = random.sample(range(0, int(args['num_of_clients'])), args['num_of_clients'])#随机抽取顺序
